...
export async function addCollection(req: Request, res: Response) {
    try {
        ...
        const savedCollection = await prisma.collection.create({ data: validation.data });
...
export async function active(req: Request, res: Response) {
    try {
        const active = await prisma.collection.findMany({where:{featured:true}});
...
export async function updateCollection(req: Request, res: Response) {
    try {
        ...
        const validation = schema.safeParse(req.body);
        if (!validation.success) return res.status(400).send({ status: 400, error: true, message: fromZodError(validation.error).message, data: {} });
        const savedCollection = await prisma.collection.update({ where: { id: id, }, data: validation.data, })
        return res.status(200).send({ status: 200, error: false, message: "ok", data: savedCollection });
    } catch (err) {
        return res.status(500).send({ status: 500, error: true, message: "Une erreur s'est produite", data: {} });
    }
