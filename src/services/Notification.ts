...
import { sendNotificationToTopic, sendPushNotification } from "../utils";
import { z } from "zod";
...
export async function sendNotification(req: Request, res: Response) {
    try {
        ...
        let result = await sendPushNotification(v_data.data.token, v_data.data.title, v_data.data.message);
...
export async function sendToTopic(req: Request, res: Response) {
    try {
        const schema = z.object({
            topic: z.string(),
            title: z.string(),
        ...
        const v_data = schema.safeParse(req.body);
        if (!v_data.success) return res.status(400).send({ error: true, message: fromZodError(v_data.error).message })
        let result = await sendNotificationToTopic(v_data.data.topic, v_data.data.title, v_data.data.message);
